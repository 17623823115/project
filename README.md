...
2.  Install Python dependencies
    ```bash
    # Install PyTorch with matching CUDA runtime
...
3.  Compile the handwritten CUDA kernel extension
    ```bash
    python setup.py build_ext --inplace
    ```
    After successful compilation, the `cuda_kernels` Python module will be generated in the current directory.

## Project Structure
```text
.
├── profile/                      # Hardware profiling scripts for Nsight Compute
│   ├── profile_gemm_native.py    # GEMM: PyTorch native baseline
│   ├── profile_gemm_cuda.py      # GEMM: Handwritten CUDA
│   ├── profile_gemm_triton.py    # GEMM: Handwritten Triton
